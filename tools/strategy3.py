...
class MomentumStrategy:
    def __init__(
        ...
        # Improvement 1: volatility penalty
        self.use_volatility_penalty = use_volatility_penalty
        self.volatility_window = volatility_window
        self.volatility_strength = volatility_strength

        self.max_price_history = max(max(self.momentum_windows) + 1, self.volatility_window + 1)
    ...
    def _update_history(self, current_market_data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Padding at the beginning to avoid empty history issue
    ...
    def _build_scores(self) -> pd.Series:
        ...
        # 2. Relative volume confirmation
        ...
        # Keep the original direction used in your previous strategy.py
        scores = -scores

        # 3. Improvement 1: volatility penalty
        if self.use_volatility_penalty and len(price_frame) >= self.volatility_window + 1:
            returns = price_frame.pct_change().dropna()
            recent_vol = returns.tail(self.volatility_window).std()
            recent_vol = recent_vol.replace([np.inf, -np.inf], np.nan).fillna(recent_vol.median())

            vol_z = self._cross_sectional_zscore(recent_vol)
            vol_z_positive = vol_z.clip(lower=0.0)

            # High-vol stocks receive lower scores.
            volatility_penalty = 1.0 / (1.0 + self.volatility_strength * vol_z_positive)
            volatility_penalty = volatility_penalty.replace([np.inf, -np.inf], 0.0).fillna(1.0)
    ...
    def _build_weights(self, scores: pd.Series) -> pd.Series:
        ...
        selected = positive_scores.nlargest(selection_count).index
    ...
    def get_selected_universe(self, current_market_data: pd.DataFrame):
        ...
        if len(self.price_history) < required_prices:
            return [], pd.Series(0.0, index=current_index), pd.Series(0.0, index=current_index)

        scores = self._build_scores()
        weights = self._build_weights(scores)

        selected_tickers = weights[weights > 0.0].index.tolist()

        return (
            selected_tickers,
            scores.reindex(current_index).fillna(0.0),
            weights.reindex(current_index).fillna(0.0),
        )

    def step(self, current_market_data: pd.DataFrame) -> pd.Series:
        selected_tickers, scores, weights = self.get_selected_universe(current_market_data)
        ...
        return weights.reindex(current_market_data.index).fillna(0.0)


class SelectedMeanVarianceStrategy(MeanVarianceStrategy):
    """
    Mean-variance optimizer inside the momentum-selected universe.
    This is for Improvement 2.
    """
    ...
    def step_selected(
        ...
        selected_tickers = [t for t in selected_tickers if t in history_df.columns]
        if len(selected_tickers) == 0:
            return weights

        history_df_sub = history_df[selected_tickers]

        # Use momentum scores as expected return proxy.
        # This avoids noisy sample mean and follows the idea:
        # momentum decides attractiveness, MV controls allocation risk.
        if alpha_scores is not None:
            ...
            # Normalize alpha scores to avoid unstable optimizer scale
            mu_norm = np.linalg.norm(mu)
            if mu_norm > 0:
                mu = mu / mu_norm
        else:
            mu, _ = self._estimate_inputs(history_df_sub)
            if mu is None:
                return weights
...
class Strategy:
    def __init__(self):
        ...
        version = 1:
            Original strategy.py + fixed alpha decay

        version = 2:
            Original strategy.py + fixed alpha decay + volatility penalty

        version = 3:
            Selected universe + 70% momentum / 30% MV

        version = 4:
            Selected universe + 80% momentum / 20% MV
        """

        self.version = 4

        # You can tune these after testing.
        self.momentum_windows = (3, 6)
        self.top_selector = 10
        self.max_weight = 0.10

        # Alpha decay setting for version 1 and 2
        self.counter = 0
        self.initial_mul_mom = 1.0
        self.final_mul_mom = 0.6

        # Correct decay horizon:
        # 78 bars per day * 5 days
        self.decay_horizon = 78 * 5
        ...
        if self.version == 1:
            ...
            self.mean_variance_strategy = SelectedMeanVarianceStrategy()
            self.hybrid_momentum_weight = 0.70
            ...
            self.mean_variance_strategy = SelectedMeanVarianceStrategy()
            self.hybrid_momentum_weight = 0.80

        else:
            raise ValueError("version must be one of {1, 2, 3, 4}")

    def _fixed_alpha_decay(self):
        decay_progress = min(1.0, self.counter / self.decay_horizon)

        mul_mom = self.initial_mul_mom - (
            self.initial_mul_mom - self.final_mul_mom
        ) * decay_progress
    ...
    def step(self, current_market_data: pd.DataFrame) -> pd.Series:
        ...
        # Version 1 and 2:
        # full-universe momentum + full-universe MV blend
        if self.version in [1, 2]:
            ...
            mul_mom = self._fixed_alpha_decay()

            weights = mul_mom * weights_mom + (1.0 - mul_mom) * weights_mv

        # Version 3 and 4:
        # selected universe + hybrid blend
        else:
            selected_tickers, scores, weights_mom = self.mom_strategy.get_selected_universe(
                current_market_data
            )

            weights_mv = self.mean_variance_strategy.step_selected(
                current_market_data=current_market_data,
                selected_tickers=selected_tickers,
                alpha_scores=scores,
            )

            beta = self.hybrid_momentum_weight
            weights = beta * weights_mom + (1.0 - beta) * weights_mv
