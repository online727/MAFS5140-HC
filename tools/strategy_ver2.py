...
class MeanVarianceStrategy:
    def __init__(self):
        ...
        Objective:
            maximize    mu^T w - (gamma / 2) * w^T Sigma_shrunk w
    ...
    def step(
        ...
        alpha_scores: pd.Series | None = None
    ) -> pd.Series:
        if "close" not in current_market_data.columns:
            raise ValueError("Input market data must contain a 'close' column.")
        ...
        # Restrict optimization to the selected universe
        ...
        if alpha_scores is not None:
            ...
            # Normalize alpha to stabilize optimizer scale
...
class MomentumStrategy:
    def __init__(self):
        ...
        print(
            f"MomentumStrategy selects top "
            f"{self._selection_count(438)}/438 assets based on momentum and volume signals."
        )
    ...
    def _update_history(self, current_market_data: pd.DataFrame):
    ...
    def _build_scores(self) -> pd.Series:
        ...
        volume_multiplier = volume_multiplier.where(
            relative_volume >= self.min_relative_volume, 0.0
        )
    ...
    def step(self, current_market_data: pd.DataFrame) -> pd.Series:
        ...
        This method now builds simple normalized momentum weights,
        but the final Strategy class uses get_selected_universe(...)
        and then runs mean-variance optimization on the selected names.
        """
